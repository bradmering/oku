...
// ── merge with whatever the author has already written ──────────────────────
// Re-running ingest must refresh the facts without touching the story. The
// author wins; ingest only adds material it has never seen. See decisions/0020.
let output: unknown = doc
const SCAFFOLD = argv.includes('--scaffold')
if (!SCAFFOLD && existsSync(OUT)) {
  const previous = yamlLoad(readFileSync(OUT, 'utf8')) as any
  if (previous?.chapters) {
    const { trip, report } = mergeTrip(doc as never, previous)
    output = trip
    console.log(`\nMerged with the existing ${OUT} — the author wins:`)
    console.log(`  ${report.preserved} chapter(s) preserved verbatim`)
    if (report.addedChapters.length) console.log(`  ${report.addedChapters.length} added for new material: ${report.addedChapters.join(', ')}`)
    for (const a of report.appendedMedia) console.log(`  ${a.mediaIds.length} new photo(s) appended to ${a.chapterId}`)
    if (report.culledStillCulled) console.log(`  ${report.culledStillCulled} previously culled photo(s) left out, as you left them`)
    if (report.unplacedMedia.length) console.log(`  ⚠️  ${report.unplacedMedia.length} new photo(s) had no chapter to join: ${report.unplacedMedia.join(', ')}`)
    if (!report.addedChapters.length && !report.appendedMedia.length) console.log('  nothing new to add')
  }
}
...
if (DRY) {
  ...
  const header =
    `# Scaffolded by scripts/ingest-trip.ts, then authored by hand or in /edit.\n` +
    `# The data proposes; the author disposes.\n` +
    `#\n` +
    `# SAFE TO RE-RUN: ingest refreshes sources and stage and adds only material it\n` +
    `# has never seen. Your prose, captions, culls and camera tweaks are preserved\n` +
    `# (decisions/0020). \`--scaffold\` bypasses that and regenerates from scratch.\n\n`
  writeFileSync(OUT, header + yamlDump(output, { lineWidth: 100, noRefs: true }))
  console.log(`\nWrote ${OUT}\n`)
}
